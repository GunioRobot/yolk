...
to file and these files are automatically rotated when they reach
a specific size. The rotation happens according to a {}``slot''
...
named \emph{Foo-rotating-INFO-1.log} (if your application is called
...
This system has the obvious advantage of never growing beyond a certain
...
Take a look at the \emph{Logging - Rotating And Regular} section of
the \emph{demo/exe/configuration/config.ini} file for more information
on the configuration parameters that affect the \emph{Yolk.Rotating\_Log}
package.

This package is used internally by various Yolk packages, so if you
use Yolk for anything at all, be sure to \emph{with} this package
and start the rotating log files. Also note that this package handles
all log data generated by the \emph{GNATCOLL.SQL} packages. See the
\emph{demo/exe/configuration/config.ini} for how to enable/disable
this output.
...
A call to \emph{Start\_Rotating\_Logs} will create three new log files
in the \emph{Yolk.Configuration.Log\_File\_Directory}. All three will
be created, even if you've disabled all \emph{GNATCOLL.SQL} related
...
won't emit a warning message on the Info track.

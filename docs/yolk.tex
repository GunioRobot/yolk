...
House-keeping is, in my humble opinion, a rather important task, and
when dealing with HTTP servers, one of those tasks involve keeping
an eye on the ever growing amount of log files. To help automate this
task we have the \emph{Yolk.Log\_File\_Cleanup} package. This package
...
Whenever the \emph{Clean\_Up} procedure is called, the \emph{Log\_File\_Directory}
is traversed and if more log files than \emph{Amount\_Of\_Files\_To\_Keep}
is found, then the oldest of these are deleted until the amount equals
\emph{Amount\_Of\_Files\_To\_Keep}.

In the demo file \emph{demo/src/yolk\_demo.adb} there's an example
on how to call this procedure using a task that wakes up according
to the configuration setting \emph{Log\_File\_Cleanup\_Interval}.
